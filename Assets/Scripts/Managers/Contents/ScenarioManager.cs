...
public class ScenarioManager
{
    ...
    public bool PassUICheck = false;
    Coroutine PassUI;
    public GameObject WearUI = null;
    ...
    IEnumerator WearingUICheck()
    {
        ...
        for (int i = 3; i > 0; i--)
        {
            ...
            popup.transform.GetChild(0).GetComponent<TMP_Text>().text = i.ToString() + "초 뒤, 개인보호구 착의법 안내 이미지가 제공됩니다.";
        ...
        if (CurrentScenarioInfo.Action == "EquipImage")
            WearUI = Managers.UI.CreateUI("WearingWay");
         
        else if (CurrentScenarioInfo.Action == "UnEquipImage")
            WearUI = Managers.UI.CreateUI("LayOutWay");

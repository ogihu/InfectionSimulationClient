...
{
    TMP_Dropdown inputPosition;
    TMP_InputField inputId;
    TMP_InputField inputPw;

    private void Awake()
    {
        inputPosition = Util.FindChildByName(gameObject, "SelectPosition").GetComponent<TMP_Dropdown>();
    ...
    {
        if (string.IsNullOrEmpty(inputPosition.options[inputPosition.value].text) ||
            string.IsNullOrEmpty(inputId.text) || string.IsNullOrEmpty(inputPw.text))
        {
            return false;
        }
        else
    ...
    public void SendAllInfo()
    {
        ...
        loginPacket.Position = inputPosition.options[inputPosition.value].text;
